...
Design tokens serve as a shared language between designers and engineers, forming the foundation for efficiently building consistent UIs. This guideline defines the types of tokens and how to use them.

## Token Architecture

We adopt a **3-layer token structure**:
...
Concrete values that serve as the lowest-layer tokens with atomic meaning.
...
3. **Expose to Tailwind**: Define `--color-*` in `@theme` as needed
4. **Document**: Add to this document

### Prohibited Practices

- Direct use of hardcoded values (use tokens instead)
- Overriding values of existing tokens
- Using meaningless abbreviations
- Direct use of Tailwind standard colors (e.g., `bg-blue-500`)
