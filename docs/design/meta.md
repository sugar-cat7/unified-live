...
Meta information is an important configuration for optimizing site display in search engines and social media. This guideline defines the setup methods for favicon, OGP, apple-touch-icon, and manifest.
...
| Size | 180 x 180 px |
| Format | PNG |
| Rounded corners | Not needed (OS applies them automatically) |
...
In addition to OGP, set meta tags specific to X (formerly Twitter).
...
Used for PWA support and icon configuration when adding to the home screen.
...
Icons specified with `purpose: "maskable"` are displayed masked into circular or other shapes on Android and similar platforms.
...
|----------|---------------|
| Public pages | Use standard OGP configuration |
