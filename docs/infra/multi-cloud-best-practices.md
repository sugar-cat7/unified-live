...
This document organizes practical guidelines for designing and operating infrastructure on AWS / GCP / Azure / Cloudflare, based on primary sources (official documentation) from each cloud provider.
Research date: 2026-02-28

## Common Principles (Shared Across All 4 Clouds)

1. **Use Well-Architected frameworks as the top-level guiding principles**
...
2. **Build the Landing Zone / foundation layer first**
   - Implement account/subscription/project isolation, IAM, networking, and log aggregation upfront.
...
4. **Separate State and permissions**
   - Isolate State per environment, and enforce short-lived credentials (OIDC, etc.) + least privilege for CI/CD.
...
2. **Organization Design (Landing Zone / Multi-account)**
   - Use AWS Control Tower + AWS Organizations as the foundation, with multi-account as the default.
   - Use "multiple accounts" as the basis for workload isolation, and avoid placing workloads in the management account.
3. **Terraform Operations**
   - Standardize the following according to AWS Prescriptive Guidance:
     - Short-lived credentials via OIDC integration (GitHub Actions/GitLab)
     - Least-privilege IAM, Secrets Manager usage
     - S3 remote backend + per-environment backend isolation
     - State versioning/audit logs (CloudTrail) and version checks in CI
...
2. **Foundation Design**
   - Reference the Enterprise foundations blueprint and standardize organization hierarchy, organization policies, log aggregation, and secret management.
3. **Terraform Operations**
   - Follow Terraform best practices (general style / reusable modules) and enforce the following:
     - Unified naming conventions (identifier consistency)
     - Purpose-based file splitting (avoid over-splitting)
     - Separation of modules and root configurations, with supplementary documents in `docs/`

### Azure

1. **Design Standards**
   - Lock in non-functional requirements using the Azure Well-Architected Framework's 5 Pillars (Reliability / Security / Cost Optimization / Operational Excellence / Performance Efficiency).
2. **Landing Zone Design**
   - Separate Platform Landing Zone and Application Landing Zone, and govern via management groups and policy inheritance.
3. **Terraform Operations**
   - Prioritize Azure Verified Modules (AVM) for Platform Landing Zones (2025-01-21) and the Azure landing zone accelerator.
   - Bootstrap by first creating the repository/pipeline/State management infrastructure, then update via CI/CD going forward.

### Cloudflare

1. **Terraform Application Policy**
   - Manage target resources centrally via Terraform following Cloudflare's official Terraform Best Practices.
2. **Configuration Isolation**
   - Separate directories by account, zone, and product unit to clarify ownership and State scope.
   - For environments (staging/QA/UAT/prod), account and domain separation is recommended.
3. **Authentication and State Management**
   - Use Account API Tokens appropriate to the use case, with permissions minimized (explicitly select Read/Edit).
...
2. Landing Zone (organization structure, networking, IAM, audit logs) initial build is complete
3. Environment isolation (dev/stg/prod) and State isolation are implemented in the IaC repository
