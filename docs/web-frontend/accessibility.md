...
| **Robust** | Interpretable by various user agents including assistive technologies |

#### WCAG 2.2 New Criteria (Level AA)

| Criterion | Description | Impact on This Project |
|-----------|-------------|----------------------|
| 2.4.11 Focus Not Obscured | Focused elements are not hidden by sticky elements | Header/navigation design |
| 2.5.7 Dragging Movements | Provide alternatives to drag operations | Future D&D feature implementation |
| 2.5.8 Target Size | Touch targets at least 24x24px | All interactive elements |
...
- **Act on the Elimination of Discrimination against Persons with Disabilities (Japan)**: Obligation to provide reasonable accommodation
...
- **Accessibility**: WCAG-compliant ARIA attributes, focus management, keyboard navigation
...
2. **Input**: Replace `<input>` with `<TextField>` + `<Input>` to integrate labels and errors
3. **Select**: Replace `<select>` with `<Select>` + `<ListBox>` to improve keyboard navigation
...
|---------|--------|-------|
| `htmlFor`/`id` pairs | ✅ Good | Form element association |
| `focus-visible` styles | ✅ Partial | Implemented in Button, Input, Select |
...
| Keyboard navigation | Medium | Navigation element focus |
...
Use appropriate HTML elements and avoid reimplementing with custom elements.
...
/* Prevent focus from being hidden by sticky elements */
html {
...
| Large text (18px or above) | 3:1 | 4.5:1 |
| UI elements/graphics | 3:1 | - |

#### Color Definition Improvements

```css
/* globals.css - Improve error/warning colors */
:root {
  /* Current: coral (oklch 0.90) - Insufficient contrast */
  /* Improved: Darker red */
  --color-error: oklch(0.55 0.20 25);     /* Approximately #D32F2F equivalent */
  --color-error-text: oklch(0.45 0.18 25); /* Darker red */

  /* Success color */
  --color-success: oklch(0.50 0.15 145);   /* Approximately #388E3C equivalent */

  /* Warning color */
  --color-warning: oklch(0.55 0.15 85);    /* Approximately #F57C00 equivalent */
}
```

#### Design That Does Not Rely on Color Alone

Combine visual cues beyond color for state changes.
...
// Error messages (notify immediately)
...
// Complex image
<figure>
  <img src="/chart.png" alt="2024 sales trend graph" aria-describedby="chart-desc" />
...
This application uses speech recognition and synthesis, so the following must be provided:

- Visual feedback for voice input (waveform display)
- Text display of AI responses (subtitle feature)
- Volume control
...
**Requirements:**
- `min-h-[44px]` to ensure target size
...
| Tool | Usage | Detection Coverage |
|------|-------|--------------------|
| axe-core | Automated checks in CI/CD | Approximately 40% |
...
1. Can all interactive elements be reached with the `Tab` key?
...
- Are form labels associated properly?
- Are image alternative texts appropriate?
...
- [ ] Close with ESC key
- [ ] Return focus to the original element after closing
...
- [ ] Verify animations with reduced-motion setting
