...
The frontend architecture adopts a Feature-based approach, organizing code by domain functionality rather than technical layers. This structure aligns with DDD principles while being adapted for frontend development. Within each Feature, the Container/Presentational pattern (Container 1st design) is implemented to separate business logic from UI rendering.

This architecture is designed for the Next.js App Router, where **Features and pages have a 1:1 correspondence**. Each route in `app/` corresponds to one Feature, keeping routing and Feature logic tightly coupled.
...
1. **Memoization is compiler-first**: Do not add `useMemo`/`useCallback` solely for speculative performance purposes
2. **Rendering is cache-first**: Define cacheable data boundaries with `'use cache'` and invalidate explicitly
3. **Dynamic boundaries are explicit**: Wrap dynamic parts with `<Suspense>` and keep the static shell on the outside
...
A Container 1st approach is adopted to separate concerns within components.
...
- API functions return `Result` from `@my-app/errors`
...
1. **Page-specific components** (`_components/`): Placed within each route, used only on that page. The `_` prefix indicates that they are private to the route and excluded from routing.
...
2. **Feature-specific components**: Placed within each Feature module for reuse within that Feature.
...
3. **Shared components**: Placed in `shared/components/` for reuse across Features.
...
In the Next.js App Router, Features and pages have a **1:1 correspondence**. Each route corresponds to one Feature.
...
1. **Feature-Page 1:1 correspondence**: Each route in `app/` corresponds to exactly one Feature. Routing and business logic are kept tightly coupled.
2. **Feature isolation**: Each Feature is self-contained with minimal dependencies on other Features. Avoid cross-Feature imports.
...
5. **Container 1st design**: Always start with a container to define what needs to be done, then create the presenter.
...
10. **Cache-first App Router**: Treat cache boundaries (`'use cache'`, `cacheLife`, `cacheTag`) as part of Feature design, not as an afterthought.
...
- **Forbidden**: Features must not import from other Features
- **Forbidden**: Shared code must not import from features or app
- Within a Feature: Container → Presenter (one-way)

### Cross-Feature Communication

Compose at the app level instead of importing between Features:
...
- Presenter tests: Test UI rendering and interaction
- Integration tests: Test the integration of Container and Presenter pairs
- E2E tests: Test entire user flows

## Implementation Guidelines

- Use TypeScript throughout the application to ensure type safety
...
Features include only the folders they need:
