...
2. **Generic errors remain unchanged**: `BAD_REQUEST`, `NOT_FOUND`, etc. are not modified
...
5. **Type-safe context**: Different context schemas are defined for each error code
...
1. **Add the error code** (`packages/errors/domain-code.ts`)
```typescript
...
2. **Define the context type** (`packages/errors/domain-context.ts`)
```typescript
...
3. **Add the user-facing message** (`services/web/shared/lib/errors/error-messages.ts`)
```typescript
