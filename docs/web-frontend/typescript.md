...
3. Use generics in utilities while keeping app code JavaScript-like
4. Ensure type flow through inference and avoid redundant annotations
5. Limit `as` to library boundaries; do not use it in app logic
...
- Isolate `as` in utils; do not use it in app logic
...
4. Prevents field omission bugs during API response transformation
...
function createProfile(): UserProfile {
  return {
    name: "John",
    // 'email' is missing - error appears at the call site
  };
}

// ✅ GOOD: satisfies detects the error immediately
function createProfile(): UserProfile {
  return {
    name: "John",
...
```typescript
// ✅ GOOD: Validates configuration shape while preserving inference
const config = {
  apiEndpoint: "/api/v1",
  timeout: 5000,
  retries: 3,
...
- Return statements containing object literals: `return {...} satisfies Type`
...
4. Include validation logic in schema definitions
